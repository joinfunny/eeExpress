...
function serviceRegister (app) {
  // release环境不走mock模式
  let serviceDirectory = app.EE_EXPRESS_CONFIG.runtime.directory.service
  serviceDirectory = path.join(path.resolve(), serviceDirectory)
  var services = Utils.file.getFiles(serviceDirectory)
  var serviceObject = {}
...
function routeRegister (app) {
  let routerDirectory = app.EE_EXPRESS_CONFIG.runtime.directory.router
  routerDirectory = path.join(path.resolve(), routerDirectory)
  var routes = Utils.file.getFiles(routerDirectory)
