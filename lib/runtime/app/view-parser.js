...
exports.use = function(app) {
  ...
  statics.forEach(directory => {
    app.use(express.static(path.join(app.rootPath, directory)))
  })

  // if `favicon.ico` exists, then set it.
  var filePath = path.join(app.rootPath, app.EE_EXPRESS_CONFIG.directory.static, 'favicon.ico')
  ...
  views.forEach(directory => {
    app.set('views', path.join(app.rootPath, directory))
  })
