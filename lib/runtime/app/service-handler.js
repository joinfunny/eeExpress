...
class Service {
  constructor(options) {
    ...
    this.proxyApiRequest = options.proxyApiRequest
  ...
  onRouter(req, res, next) {
    ...
    if (that.beforeExecute(req, res)) {
      ...
      that.options.request = that.options.request || that.proxyApiRequest
      that.options.request.call(that, req, res, function(actionResult) {
...
module.exports = (app, ServiceObject) => {
  let proxyApiRequest = apiService(app)
  new ServiceGenerator(app, ServiceObject).register(proxyApiRequest)
