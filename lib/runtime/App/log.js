...
function parse(logConfig) {
  var serviceName = process.env.NODE_SERVICE || 'default'
  if (serviceName) {
  ...
  // check config directory had exists ,if not exists,then create it.
...
exports.use = function(app) {
  if (app.EE_EXPRESS_CONFIG.disableLog) return
  var logger = parse(app.EE_EXPRESS_CONFIG.log4js)
