...
class ServiceDirector {
  ...
  onRouter (req, res, next) {
    ...
    if (that.beforeExecute(req, res)) {
       ...
       */
      if (process.env.ENV !== 'production' && typeof (req.query.mock) !== 'undefined') {
        that.onMocking(req, res, that.options.mock)
      }

      that.options.request && that.options.request(req, res, function (actionResult) {
  ...
  beforeExecute (request, response) {
    ...
    if (!validResult.valid) {
      ...
      beforeRequest && _.isFunction(beforeRequest) && beforeRequest(request, response)
    }
  ...
  afterExecute (request, response, actionResult) {
    ...
    afterRequest && _.isFunction(afterRequest) && afterRequest(request, response, actionResult)
  }
  /**
   * Mock数据
...
function registerServices (services) {
  for (let url in services) {
    ...
    service.url = url
    ServiceDirector.register(new ServiceDirector(service))
  }
}

module.exports = ServiceObject => {
  registerServices(ServiceObject)
  registerServices(apiService)
  return generator
}
