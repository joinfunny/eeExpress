...
class EEExpress {
  ...
  setConfig(config) {
    this.config = this.app.EE_EXPRESS_CONFIG = _.cloneDeep(
      deepAssign({}, defaultConfig, runtime.app.config(this.app), this.config, config)
    );
  ...
  authRegister(filters) {
    ...
    eeExpressCache[this._instanceId] = _.extend({}, eeExpressCache[this._instanceId], {authFilters: theFilters})
  }
  /**
   * customic proxy api service hander register
   * @param {*Function} apiServiceFunc
   */
  proxyApiServiceRegister(apiServiceFunc) {
    if (!apiServiceFunc && Object.toString.call(apiServiceFunc) !== '[object Function]') {
      throw new ReferenceError("you must set a apiService Function.");
    }
    eeExpressCache[this._instanceId] = _.extend({}, eeExpressCache[this._instanceId], {proxyApiService: apiServiceFunc})
  }
  init() {
    ...
    // authorize setting
    runtime.app.auth.use(this.app, eeExpressCache[this._instanceId].authFilters || []);

    //first module register
    runtime.app.modules.use(this.app)
    //then router & service register
    runtime.app.routeRegister.use(this.app, eeExpressCache[this._instanceId].proxyApiService);
  }
  run() {
    let app = this.app;
